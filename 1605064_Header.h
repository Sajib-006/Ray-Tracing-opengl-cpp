...
extern double clipColor(double color);
...
public:
    ...
    double intersect(Ray *r, double *color_out, int level){
        ...
        if(dbg) cout<< "In intersect(): ";
        discrim = b * b - 4 * val;
        //cout<<"val "<<val<<" b:"<<b<<"discrim: "<<discrim<<endl;
        //R0.print();
        if(dbg) r->dir.print();
        if(dbg) cout<<"discrim: "<<discrim<<endl;
            ...
            if(level == 0) return t;
                ...
                for(int i=0; i<lights.size(); i++){
                    ...
                    if(!obscured){
                        ...
                        r_ray_dir = L - N*(2.0 * L.dotProduct(N));
                        Ray *r_ray = new Ray(ip,r_ray_dir);
                        if (debugT) fout<<"ray r made"<<endl;
                        R = r_ray->dir;
                        lambert = max(0.0, L.dotProduct(N));
                        V = r->dir;
                        phong = max(0.0, R.dotProduct(V));
...
public:
    ...
    Floor(double floorWidth, double tileWidth){
        texture = bitmap_image("1605064.bmp");
    ...
    void setIpColor(Vector3D int_point){
        if(int_point.x >= reference_point.x && int_point.x <= -reference_point.x && int_point.y >= reference_point.y && int_point.y <= -reference_point.y){
            double x_val = int_point.x/length;
            double y_val = int_point.y/length;
            int val = int(ceil(x_val)) + int(ceil(y_val));
            if( val%2 == 0 ) {
                color[0] = 1;
                color[1] = 1;
                color[2] = 1;
                ...
                if(!textureOn){
                    ...
                    int px,py;
                    unsigned char r,g,b;
                    px = int(abs(int_point.x)) % int(length);
                    py = int(abs(int_point.y)) % int(length);
                    //cout<<px<<" "<<py<<endl;
                    texture.get_pixel(px,py,r,g,b);
                    color[0] = double(r);
                    color[0] = double(g);
                    color[0] = double(b);
    ...
    double intersect(Ray *r, double *color_out, int level){
        ...
        if(level == 0 ) return t;
            ...
            for(int i=0; i<lights.size(); i++){
                ...
                Vector3D l_ray_start = ip + eps;
                Ray *l_ray = new Ray(l_ray_start, lights[i]->light_pos - ip);
                if(debugF) fout<<"l ray made"<<endl;
                double dist = (lights[i]->light_pos - l_ray_start).getValue();
                ...
                if(!obscured){
                    ...
                    r_ray_dir = L - N*(2.0 * L.dotProduct(N));
                    Ray *r_ray = new Ray(ip,r_ray_dir);
                    if(debugF) fout<<"r ray made"<<endl;
                    R = r_ray->dir;
                    lambert = max(0.0, L.dotProduct(N));
                    V = r->dir;
                    phong = max(0.0, R.dotProduct(V));
        ...
        if(ax == 0.0f) {
            ...
            if(level == 0) return t;
                ...
                for(int i=0; i<lights.size(); i++){
                    ...
                    Vector3D l_ray_start = ip + eps;
                    Ray *l_ray = new Ray(l_ray_start, lights[i]->light_pos - ip);
                    if (debugG) fout<<"l ray made"<<endl;
                    double dist = (lights[i]->light_pos - l_ray_start).getValue();
                    ...
                    if(!obscured){
                        ...
                        r_ray_dir = L - N*(2.0 * L.dotProduct(N));
                        Ray *r_ray = new Ray(ip,r_ray_dir);
                        if (debugG) fout<<"r ray made"<<endl;
                        R = r_ray->dir;
                        lambert = max(0.0, L.dotProduct(N));
                        V = r->dir;
                        phong = max(0.0, R.dotProduct(V));
